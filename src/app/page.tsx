...
export default function HomePage() {
  ...
  const [customMaps, setCustomMaps] = useState<string[]>(MAP_POOL.map((m) => m.id));
  ...
  useEffect(() => {
    if (customFormat && customMaps.length < minMapsForBestOf(bestOfForFormat(customFormat))) {
  ...
  const createRoom = async () => {
    ...
    if (tab === "presets") {
      ...
      if (customMaps.length < minMapsForBestOf(bestOfForFormat(customFormat))) return;
      config = {
        presetId: "custom",
        mapPool: customMaps,
        format: customFormat,
    ...
    router.push(getTeamHref(sessionId, teamAToken));
  };
  ...
  const summary =
    tab === "presets" && selectedPresetId
      ? `${groups.find((g) => g.stages.some((s) => s.presetId === selectedPresetId))?.name ?? "Preset"} · ${
          FORMAT_LABEL[
            groups.flatMap((g) => g.stages).find((s) => s.presetId === selectedPresetId)?.format ?? ""
          ] ?? ""
        }`
        ...
        <div className="mx-auto max-w-2xl px-6 py-16">
          ...
          <div className="mb-12 text-center">
            ...
            <h1 className="mt-3 font-serif text-4xl font-medium leading-[1.1] tracking-tight">
              Run a clean pick-ban, <em className="text-muted-foreground">in real time</em>.
            </h1>
            <p className="mx-auto mt-4 max-w-md text-sm text-muted-foreground">
              Choose a preset and share links with both teams. Results are saved locally and can be
              exported later.
            </p>
          </div>
                ...
                {groups.map((group) => (
                  <div key={group.id} className="rounded-lg border border-border bg-card p-4">
                    <div className="flex items-baseline justify-between gap-3">
                      ...
                      <span className="text-[11px] text-muted-foreground">by {group.author}</span>
                    </div>
                    ...
                    {FORMATS.map((f) => {
                      ...
                      return (
                        <button
                          ...
                          title={available ? `Plays ${f.plays}` : `Needs at least ${min} maps`}
            ...
            <div className="overflow-hidden rounded-lg border border-border bg-card">
              <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {RANKED_MAP_POOL.length} maps
                </span>
              </div>
              <div className="grid grid-cols-2">
                {RANKED_MAP_POOL.map((map, i) => (
                  <div
                    key={map.id}
                    className={`flex items-center gap-2 px-4 py-2 text-sm ${
                      i < RANKED_MAP_POOL.length - 2 ? "border-b border-border" : ""
                    }`}
                  >
