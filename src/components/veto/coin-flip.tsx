...
function CoinFace({ label, sub, back }: { label: string; sub: string; back?: boolean }) {
  return (
    <div
...
export function CoinFlip({
  ...
  const [phase, setPhase] = useState<Phase>(coinFlip ? "settling" : "waiting");
  const onRevealRef = useRef(onRevealComplete);
  useEffect(() => {
    onRevealRef.current = onRevealComplete;
  });

  const seed = coinFlip?.seed ?? null;
  ...
  useEffect(() => {
    ...
    // Keyed on seed so re-renders that produce a new coinFlip object identity
    // (same logical flip) don't restart the animation.
  ...
  if (seededPick && !coinFlip) {
    ...
    return (
      <Card className="w-full max-w-md space-y-4 p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">Choose who picks/bans first.</p>
        <div className="flex justify-center gap-3">
          <Button type="button" onClick={() => onChooseFirstActor?.("a")} aria-label={`${teamNames.a} first`}>
            {teamNames.a} first
          </Button>
          <Button type="button" onClick={() => onChooseFirstActor?.("b")} aria-label={`${teamNames.b} first`}>
            {teamNames.b} first
          </Button>
        </div>
      </Card>
  ...
  const spinning = phase === "waiting" || !coinFlip;
      ...
      <div className="[perspective:900px]">
        <div
          ...
          style={
            spinning
              ? undefined
              : { transform: restTransform, transition: `transform ${SETTLE_MS}ms cubic-bezier(.18,.7,.2,1)` }
          }
          aria-label={spinning ? "Flipping coin" : `${winner === "a" ? teamNames.a : teamNames.b} won the flip`}
        >
        ...
        {phase === "revealed" && coinFlip ? (
          <div className="space-y-1" aria-live="polite">
            ...
            <p className="text-xs text-muted-foreground">
              {coinFlip.method === "seeded-pick" ? "Seeded — chosen by Team A" : "Random coin flip"}
            </p>
          </div>
        ) : (
