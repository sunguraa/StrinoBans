    ...
    <div className="w-full max-w-xl space-y-5">
          ...
          {(["a", "b"] as Team[]).map((team) => {
            ...
            return (
              <TeamPanel
                ...
                onReadyToggle={role === team && !isEmpty ? () => onReadyToggle(team) : undefined}
              />
            );
          })}
        </div>
      ...
      {links && !bothReady && (
        <Card className="p-6 space-y-3">
          <h3 className="font-semibold text-sm text-muted-foreground tracking-wide uppercase">Share Links</h3>
          ...
          <div className="space-y-2">
            ...
            {showSpectator && <LinkRow label="Spectator Link" url={links.spectator} />}
          </div>
        </Card>
      )}
    </div>
