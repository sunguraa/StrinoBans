...
export function ShareModal({ links, roomImportCode, onClose }: ShareModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label="Share room links">
      <Card className="w-full max-w-xl space-y-4 p-6">
        ...
        <p className="text-sm text-muted-foreground">Send each team their private link. Spectators can watch in real time.</p>
        <div className="space-y-3">
          {roomImportCode && (
            <CopyRow label="Room code" url={roomImportCode} />
          )}
