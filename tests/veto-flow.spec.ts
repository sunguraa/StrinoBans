...
test('landing shows the Default preset group and creates a room with a correct URL', async ({
  ...
  for (const bo of ['Bo1', 'Bo3', 'Bo5', 'Bo7']) {
    await expect(page.getByRole('button', { name: `Default ${bo}` })).toBeVisible();
  }
...
test('custom map pool gates the available best-of formats by size', async ({ page }) => {
  await page.goto(APP);
  ...
  await expect(page.getByRole('button', { name: 'Create veto room' })).toBeEnabled();
});

test('full Bo1 veto runs end to end across two peers', async ({ page, context }) => {
  const teamA = page;
  ...
  // Pull Team B's private link out of the share modal, then open it as a second peer.
  const teamBUrl = await teamA.getByRole('textbox', { name: 'Team B link' }).inputValue();
  await teamA.getByRole('button', { name: 'Close' }).click();

  const teamB = await context.newPage();
  await teamB.goto(teamBUrl);
  // A team link always opens the share modal once links sync — wait for it and close.
  await teamB.getByRole('button', { name: 'Close' }).click();
  ...
  await expect(teamA.getByRole('heading', { name: 'Veto Complete' })).toBeVisible();
  await expect(teamB.getByRole('heading', { name: 'Veto Complete' })).toBeVisible();
...
async function actIfMyTurn(p: Page): Promise<boolean> {
  ...
  const attacker = p.getByRole('button', { name: 'Choose attacker', exact: true });
  ...
  if ((await confirm.isVisible().catch(() => false)) && (await maps.count()) > 0) {
...
async function driveToCompletion(pages: Page[]): Promise<void> {
  for (let i = 0; i < 40; i++) {
    for (const p of pages) {
      if (await p.getByRole('heading', { name: 'Veto Complete' }).isVisible().catch(() => false)) {
        return;
      }
    }
    let acted = false;
